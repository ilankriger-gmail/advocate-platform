...
async function getRankingData(currentUserId: string) {
  ...
  const { data: rankingTotalData } = await supabase.rpc('get_ranking_total');

  if (error || !rankingData) {
    console.error('Error fetching ranking:', error);
    return { ranking: [], rankingTotal: [], userPosition: null, userBalance: 0, userTotalEarned: 0 };
  }
  ...
  // Montar ranking total (acumulado)
  const rankingTotal = (rankingTotalData || []).map((entry: { user_id: string; full_name: string | null; avatar_url: string | null; email: string | null; balance: number; total_earned: number }, index: number) => ({
    position: index + 1,
    userId: entry.user_id,
    fullName: entry.full_name || (entry.email ? entry.email.split('@')[0] : `Membro #${index + 1}`),
    username: null,
    avatarUrl: entry.avatar_url || null,
    balance: entry.total_earned || entry.balance || 0,
    isCurrentUser: entry.user_id === currentUserId,
    previousPosition: previousMap.get(entry.user_id) || null,
  }));

  // Montar ranking com posições (disponível)
  const ranking = rankingData.map((entry: { user_id: string; full_name: string | null; avatar_url: string | null; email: string | null; balance: number; total_earned: number }, index: number) => ({
    position: index + 1,
    userId: entry.user_id,
    fullName: entry.full_name || (entry.email ? entry.email.split('@')[0] : `Membro #${index + 1}`),
    username: null,
  ...
  const userTotalIndex = rankingTotal.findIndex((r: { isCurrentUser: boolean }) => r.isCurrentUser);
  const userTotalEarned = userTotalIndex >= 0 ? rankingTotal[userTotalIndex].balance : 0;

  return { ranking, rankingTotal, userPosition, userBalance, userTotalEarned };
}
...
export default async function RankingPage() {
  ...
  const { ranking, rankingTotal, userPosition, userBalance, userTotalEarned } = await getRankingData(user.id);
