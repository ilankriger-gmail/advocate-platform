-- Migration: Corrigir role nas views de leaderboard
-- Bug: Views usavam role='user' mas o correto é role='fan'

-- ============================================
-- CORRIGIR VIEWS PARA LEADERBOARD
-- ============================================

-- View: Ranking por moedas (corrigido para role='fan')
...
-- View: Ranking combinado (corrigido para role='fan')
...
-- ============================================
-- CORRIGIR FUNÇÕES PARA RANKING DO USUÁRIO
-- ============================================

-- Função: Ranking do usuário por moedas (corrigido)
...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      ELSE 'bronze'
    END as tier,
    (SELECT COUNT(*) FROM users WHERE role = 'fan') as total_users
  FROM ranked r
  WHERE r.id = p_user_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Função: Ranking do usuário combinado (corrigido)
...
-- Função: Ranking do usuário por desafios (corrigido)
...
-- Função: Ranking do usuário por eventos (corrigido)
